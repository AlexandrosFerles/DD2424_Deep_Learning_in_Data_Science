...
def load_test():
    Xtest, y_test, Ytest=LoadBatch('../Datasets/test_batch')
...
def svm_cost(W, X, y):
    ...
    for i in range(num_train):
        ...
        elem = y[i]
        try:
            correct_class_score = scores[elem]
        except IndexError:
            print(elem)
...
def grad_check_sparse(f, x, analytic_grad, num_checks):
    ...
    for i in xrange(num_checks):
        ...
        print
        'numerical: %f analytic: %f, relative error: %e' % (grad_numerical, grad_analytic, rel_error)
...
def get_mini_batches_svm(Xtrain, y_train):
    Xbatches = []
    y_batches = []
    for batch in range(1, int(Xtrain.shape[1] / N_BATCH)):
        start = (batch - 1) * N_BATCH + 1
        end = batch * N_BATCH + 1
