...
def mini_batch(X, Y, y_tr, X_val, Y_val_hot, y_val, GDparams, W, b, lamda):
    ...
    for epoch in range(GDparams.n_epochs):
        ...
        while i < len(y_tr):
            ...
            if mode == 'softmax':
                J_grad_W, J_grad_b = compute_gradients(X_batch, Y_batch, None, evaluate_classifier(X_batch, W, b), W, b,
                                                       lamda)
            elif mode == 'SVM':
                J_grad_W, J_grad_b = compute_gradients(X_batch, Y_batch, y_batch_tr, None, W, b, lamda)
            W -= GDparams.eta * J_grad_W
...
def main():
    # Load dataset
    print("Loading data ...")
    X_train, Y_train_hot, y_train = load_data('../Datasets/data_batch_1')
    X_val, Y_val_hot, y_val = load_data('../Datasets/data_batch_2')
    ...
    # --- Mini-Batch
    GDparams = GD_params(n_batch=100, eta=0.01, n_epochs=40)
    start = default_timer()
    # Exercise 1:
    ...
    J_train_ls, J_val_ls, loss_train_ls, loss_val_ls, acc_train_ls, acc_val_ls, W_star, max_val_acc = mini_batch(
        X_train, Y_train_hot, y_train, X_val, Y_val_hot, y_val, GDparams, W, b, lamda)
